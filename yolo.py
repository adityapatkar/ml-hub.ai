...
def detect_objects(our_image):
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            cv2.putText(img, label, (x+20,y+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            cv2.putText(img, str(conf), (x+20,y+40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)    
            items.append(label)
...
def face_main():
    ...
    if choice == "Browse an Image":
        ...
        if image_file is not None:
            ...
            our_image = Image.open("images/Zinedine.jpg")
            detect_objects(our_image)
...
def object_main():
    ...
    if choice == "Browse an Image":
        ...
        our_image = Image.open("images/man-bird.jpg")
        detect_objects(our_image)
